 ...
 *               args:
 *                 type: array
 *                 description: Optional arguments passed to the expression.
 *               awaitResult:
 *                 type: boolean
 *                 description: Whether to await the result (default true).
 *     responses:
 *       200:
...
app.post('/tabs/:tabId/evaluate', authMiddleware(), express.json({ limit: '1mb' }), async (req, res) => {
  ...
  try {
    ...
    pluginEvents.emit('tab:evaluate', { userId, tabId, expression });
    const result = await withUserLimit(userId, () => withTabLock(tabId, async () => {
      if (awaitResult) {
    ...
    pluginEvents.emit('tab:evaluated', { userId, tabId, result });
